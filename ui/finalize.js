...
function addLineBreaks(buffer) {
  let data = "";
  let chunks = chunkArray(buffer, 64);
...
`;
    ...
    FS.writeFileSync(
      path.resolve(__dirname, SAVE_PATH + "/webserial_webpage.h"),
      FILE
    );
    console.log(
      `[COMPRESS.js] Compressed Bundle into webpage.h header file | Total Size: ${(
